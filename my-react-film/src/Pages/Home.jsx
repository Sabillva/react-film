...
const Home = ({ savedLists, setSavedLists }) => {
  ...
  const fetchMovies = useCallback(() => {
    ...
    if (lastQuery !== "") {
      fetch(
        `https://www.omdbapi.com/?apikey=522f2f8f&s=${lastQuery}&type=movie`
      )
  ...
  const saveList = () => {
    ...
    const isDuplicateName = savedLists
      .map((list) => list.name)
      .includes(selectedList);
      ...
      <div className="container">
        ...
        <div className="container-right">
          <div className="list-area">
            ...
            />
            {!isListSaved && (
              <button className="btn-primary-add" onClick={saveList}>
                Save List{" "}
              </button>
            )}
            <Link to="/lists">
              <button className="btn-primary-add">Go to Lists</button>
            </Link>
